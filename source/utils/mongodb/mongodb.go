...
func GetLatestPunkIndex() map[string]interface{} {
	if client != nil {
		...
		fmt.Println(resultsFiltered[len(resultsFiltered)-1]["price"])
		fmt.Println(resultsFiltered[len(resultsFiltered)-1]["timestamp"])
...
func GetPunkIndexHistoryDaily() []interface{} {
	if client != nil {
		...
		for _, result := range resultsFiltered {
			...
			if strings.Contains(timeT.String(), "01:00") {
