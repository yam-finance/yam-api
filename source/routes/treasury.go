...
func GetTreasury(geth *ethclient.Client) map[string]interface{} {
	...
	dpiBalanceMultisig := utils.GetBalance(contractAddress.DPI, contractAddress.MultisigAddress, geth, 18)
	dpiBalanceTreasury := utils.GetBalance(contractAddress.DPI, contractAddress.TreasuryAddress, geth, 18)
	dpiBalance := new(big.Float).Add(dpiBalanceTreasury, dpiBalanceMultisig)
	dpiPrice := utils.GetValue("defipulse-index")
	dpiValue := CalculateValue(dpiBalance, dpiPrice)
	umaBalanceTreasury := utils.GetBalance(contractAddress.UMA, contractAddress.TreasuryAddress, geth, 18)
	umaBalanceTwoKey := utils.GetBalance(contractAddress.UMA, contractAddress.UmaTwoKeyAddress, geth, 18)
	umaBalance := new(big.Float).Add(umaBalanceTreasury, umaBalanceTwoKey)
	...
	sushiBalanceMultisig := utils.GetBalance(contractAddress.SUSHI, contractAddress.MultisigAddress, geth, 18)
	sushiBalance := new(big.Float).Add(sushiBalanceTreasury, sushiBalanceIncentivizer, sushiBalanceMultisig)
	sushiPrice := utils.GetValue("sushi")
	sushiValue := CalculateValue(sushiBalance, sushiPrice)
	xsushiBalanceTreasury := utils.GetBalance(contractAddress.XSUSHI, contractAddress.TreasuryAddress, geth, 18)
	xsushiBalanceMultiSig := utils.GetBalance(contractAddress.XSUSHI, contractAddress.MultisigAddress, geth, 18)
	xsushiBalance := new(big.Float).Add(xsushiBalanceTreasury, xsushiBalanceMultiSig)
	...
	indexBalanceMultisig := utils.GetBalance(contractAddress.INDEX, contractAddress.MultisigAddress, geth, 18)
	indexBalance := new(big.Float).Add(indexBalanceTreasury, indexBalanceTimelock, indexBalanceMultisig)
	...
	gtcBalanceTreasury := utils.GetBalance(contractAddress.GitCoinAddress, contractAddress.TreasuryAddress, geth, 18)
	gtcBalanceMultisig := utils.GetBalance(contractAddress.GitCoinAddress, contractAddress.MultisigAddress, geth, 18)
	gtcBalance := new(big.Float).Add(gtcBalanceTreasury, gtcBalanceMultisig)
	...
	ethdpilpBalanceTreasury := GetETHDPILPBalance(geth)
	ethdpilpBalanceMultisig := utils.GetBalance(contractAddress.UnilpETHDPI, contractAddress.MultisigAddress, geth, 18)
	ethdpilpBalance := new(big.Float).Add(ethdpilpBalanceTreasury, ethdpilpBalanceMultisig)
